...
router.get('/home/:customerId', async (req, res, next) => {
  try {
    ...
    const trending = await getTrending(12);
    return res.json({
      source: 'trending',
      customerId,
      items: withReason(
...
router.get('/trending', async (_req, res, next) => {
  try {
    const trending = await getTrending(12);
    res.json({
      items: withReason(hydrate(trending), 'Top bán chạy toàn hệ thống'),
    });
  } catch (err) {
    next(err);
  }
...
router.post('/cart', async (req, res, next) => {
  try {
    ...
    const stats = new Map();
    const cartRecsLists = await Promise.all(items.map((id) => getCartRecs(id)));
    items.forEach((id, idx) => {
      const recs = cartRecsLists[idx];
      recs.forEach((rid, i) => {
